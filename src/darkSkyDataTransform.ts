...
const getDarkSkyData = async (
  ...
  path: string,
  token: string,
  coordinate: IInputRow) => {
  console.log(`Getting data for lat: ${coordinate.lat} long:_
  ${coordinate.long} from ${hostname} at ${coordinate.time}`);
  const coordinateTime = coordinate.time;
  const formattedRequestURLForCoordinateAsString =
    `${protocol}://${hostname}/${path}/${token}/${coordinate.lat}_
  ,${coordinate.long},${coordinateTime}?exclude=flags,minutely,currently&units=si`;
